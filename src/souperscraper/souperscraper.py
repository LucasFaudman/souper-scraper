...
class SouperScraper:
    def __init__(self,
        ...
        if not executable_path.exists():
            raise FileNotFoundError(f"Executable path {executable_path} does not exist. Use souperscraper.get_chromedriver() to download chromedriver.")
        ...
        self.soup_parser = soup_parser # Save for later to use when calling SoupScraper.soup
    ...
    def __getattr__(self, attr):
        ...
        split_attr = attr.split('_') # Split attribute by '_' to check for 'soup', 'by', 'wait', etc.
